...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    ...
    while True:
      ...
      for index,value in enumerate(classes[0]):
        ...
        if scores[0,index] > 0.3:
          ...
          if category_index.get(value).get('name')=='person' or category_index.get(value).get('name')=='pigeon' or category_index.get(value).get('name')=='eurasicanTreeSparrow':
            ...
            harmful = 0
            firstdetection = category_index.get(value).get('name')
      ...
      print('harmfulcount=%d'%harmfulcount)
      print('harmfulcountbird=%d'%harmfulcountbird)
      print('harmfulcountship=%d'%harmfulcountship)
      print('unharmfulcount=%d'%unharmfulcount)
      random_num = random.randrange(1,4)
      print('random_number=%d'%random_num)  
      if (harmful==0 and unharmfulcount >= 1) or (harmful==0 and unharmfulcount==0): 
        if play==1:
            ...
            elapsed2 = start - end2
            if elapsed > 30:
                print('over the 30 seconds and still harmful objects are exist.')
                cv2.imwrite("/var/www/html/imgs/" + str(getname2) + " (Date: " + str(now) + ").jpg", image_np)
                interval = time.time()
                end = interval
            elif elapsed < 30 and getname != getname2 :
                print('different harmful object is detected in 30 seconds')
                cv2.imwrite("/var/www/html/imgs/" + str(getname2) + " (Date: " + str(now) + ").jpg", image_np)
                if getname == 'fishBoat' and (getname2 == 'baikalTeal' or getname2 == 'greyHeron' or getname2 == 'cormorant'):
                  pygame.mixer.music.stop()
                  print('fishBoat --> harmfulbird')
                  if random_num==1 :
                    ...
                    pygame.mixer.music.play(-1)
                elif (getname == 'baikalTeal' or getname == 'greyHeron' or getname == 'cormorant') and getname2 == 'fishBoat' :
                  pygame.mixer.music.stop()
                  print('harmfulboat --> fishBoat')
          ...
          if capcount ==0:
            ...
            cv2.imwrite("/var/www/html/imgs/" + str(getname2) + " (Date: " + str(now) + ").jpg", image_np)
            print('this is a first harmful capture')
            capcount = 1
            print('capcount=%d'%capcount)
            end = time.time()
            end2 = time.time()
            getname = firstdetection
            if harmfulcountship >= 1 and harmfulcountbird == 0 : 
                push='1'
                url = "http://localhost/fcm/push_notification_ship.php?push="+push
                response = urllib.request.urlopen(url)
            ...
            if elapsed > 30:                           #every 30seconds, capture the image
              cv2.imwrite("/var/www/html/imgs/" + str(getname2) + " (Date: " + str(now) + ").jpg", image_np)
              print('over the 30 seconds and harmful objects appeared!')
              interval = time.time()
              end = interval
            elif elapsed < 30 and getname != getname2 :
              cv2.imwrite("/var/www/html/imgs/" + str(getname2) + " (Date: " + str(now) + ").jpg", image_np)
      ...
      cv2.imshow('object detection',cv2.resize(image_np,(1200,1000)))
